 ...
 */
export function deleteData(keyValue) {
  if (window.confirm("Alert! This action will clear entire row!")) {
 ...
 * input.
 */
 export function disableWarning_userName() {
 ...
 */
export function disableWarning_regNo() {
  document.getElementById("regNoWarning").innerHTML = "";
}

/**
 * The function disables the warning message for a grade.
 */
export function disableWarning_grade() {
 ...
 */
export function editData(keyValue) {
  if (!statusMode.localeCompare("normal")) {
    statusMode = "editing";
    document
 ...
 */
export function onInputChange(keyValue) {
 ...
 * editing process.
 */
export function cancelEdit(keyValue) {
  statusMode = "normal";
  let row = document.getElementById(`row${keyValue}`);
  let dataIndex = tableData.findIndex((object) => {
    return object.keyValue === keyValue;
  });
  for (i = 1; i < row.cells.length - 1; i++) {
 ...
 */
export function updateData(keyValue) {
  statusMode = "normal";
  let row = document.getElementById(`row${keyValue}`);
  let dataIndex = tableData.findIndex(object => {
              return object.keyValue === keyValue
  ...
  for (i = 1; i < row.cells.length - 1; i++) {
    let inputvalue = document.getElementById(`row${keyValue}`).cells[i].childNodes[0].value
...
export function searchData(){
     let searchValue = document.getElementById('search').value.toString().trim()
 ...
 */
export function filterBySearch(arr, searchKey){
...
/**
 * The function displays a warning message for the user to enter a valid name consisting of only
 * alphabets.
 */
export function Warning_userName() {
  document.getElementById("userNameWarning").innerHTML = "Enter the Valid Name(Hint Alphabets Only)";
}
 ...
 * 100.
 */
export function Warning_grade() {
  document.getElementById("gradeWarning").innerHTML = "Enter Numbers from 0 to 100";
}
 ...
 */
export function clearTheTable(){
  displayTable(tableData)
    if (window.confirm("Alert! This action will clear all data!")) {
        clearForm()
        clearSearchData()
        tableData = []
        displayTable(tableData)
